...
std::size_t GroceryList::find( const GroceryItem & groceryItem ) const
{
  ...
  for (int i = 0; i < _gList_vector.size(); i++){
    if (_gList_vector[i] == groceryItem){
...
void GroceryList::insert( const GroceryItem & groceryItem, std::size_t offsetFromTop )        // insert provided grocery item at offsetFromTop, which places it before the current grocery item at offsetFromTop
{
  ...
  { /**********  Part 1 - Insert into array  ***********************/
    ...
    if(_gList_array_size >= _gList_array.size()) throw CapacityExceeded_Ex( "Valid elemets can't be over the size of array");
    
    //template <GroceryList, GroceryList> inline GroceryList 
    //std::move_backward<GroceryList, GroceryList>(List._gList_array[offsetFromTop], List._gList_array[(List._gList_array_size)], List._gList_array[(List._gList_array_size) + 1] );
    for(int i = _gList_array_size; i >= offsetFromTop; i--){

      GroceryItem tmp;

      if(i == offsetFromTop){

          tmp = _gList_array[i+1];

          _gList_array[i+1] = _gList_array[i];

          _gList_array[i] = groceryItem;
      } 
...
void GroceryList::remove( std::size_t offsetFromTop )
{
  ...
  { /**********  Part 1 - Remove from array  ***********************/
    ///////////////////////// TO-DO (8) //////////////////////////////
      ...
      /// in your array, so don't forget to adjust _gList_array_size.
    std::move(_gList_array.begin(), _gList_array.begin() + _gList_array_size, _gList_array);
    /////////////////////// END-TO-DO (8) ////////////////////////////
  } // Part 1 - Remove from array
