...
int main()
{
  try
  {
    ...
    MyList.moveToTop({"marinara sauce"});

    std::cout << "What do I have to buy?: \n" << MyList << "\n\n";

    MyList += {{"cream sauce"},
                {"raviolli"}};

    GroceryList NeedList = {{"pasta"},
                            {"onion"},
                            {"pickle"},
                            {"cracked pepper"}};

    std::cout << "By the way, what do I really need?:\n" << NeedList << "\n\n";

    if(MyList != NeedList) MyList += NeedList;

    MyList.insert({"milk", "Lactaid"}, MyList.find({"baguette"}));

    MyList.remove({"baguette"});
